...
import { CHAINS, CHARMS } from "@/lib/products";
...
export default function BuilderPage() {
  ...
  const available = CHARMS.filter((c) => !c.soldOut);
  const total = chain.price + charms.reduce((n, c) => n + c.price, 0);
  ...
  function addToCart() {
    addItem({ slug: chain.slug, name: chain.name, price: chain.price, metal, tone: chain.tone });
    charms.forEach((c) =>
      addItem({ slug: c.slug, name: c.name, price: c.price, metal, tone: c.tone })
    );
    setAdded(true);
    ...
    <div className="mx-auto max-w-6xl px-5 py-14">
      ...
      <div className="grid gap-12 md:grid-cols-[1fr_1.1fr]">
        ...
        <div className="md:sticky md:top-24 md:self-start">
          ...
          <button
            onClick={addToCart}
            className="mt-4 w-full rounded-full bg-olive py-3.5 text-sm text-cream transition hover:bg-olive-deep"
          >
